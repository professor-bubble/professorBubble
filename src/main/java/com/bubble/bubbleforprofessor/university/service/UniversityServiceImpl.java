...
public class UniversityServiceImpl implements UniversityService {
    ...
    public void init(){
        ...
        saveAllUniversities(UniversityApiRequest.builder().build())
                .subscribe(null, error -> log.error("초기 로드 중 오류 발생: {}", error.getMessage()));
    ...
    public void universityUpdate(){
        ...
        if(today.getMonthValue() == 11 && today.getDayOfMonth() == 22){
            ...
            saveAllUniversities(UniversityApiRequest.builder().build())
                    .subscribe(null, error -> log.error("업데이트 중 오류 발생: {}", error.getMessage()));
        }
    }
    ...
    public Mono<Void> saveAllUniversities(UniversityApiRequest uniRequest) {
        ...
        return webClient.get()
                .uri(uriBuilder -> uriBuilder
                        ...
                        .queryParam("Fclty_Cd", initRequest.getFcltyCd())
                        .build())
...
//                dbUniversityMap.remove(apiUniversity.getUniversityId());
                ...
                dbUniversity.updateNameIfChanged(apiUniversity.getUniversityName());
                dbUniversity.restoreIfDeleted();
                universitiesToSave.add(dbUniversity);
                dbUniversityMap.remove(apiUniversity.getUniversityId());
            }
        }
        ...
        for (University deletedUniversity : dbUniversityMap.values()) {
            ...
            deletedUniversity.markAsDeleted();
    ...
    public Mono<Integer> onelist(UniversityApiRequest request) {
        UniversityApiRequest newRequest = UniversityApiRequest.builder()
                .serviceKey(request.getServiceKey())
                .pageNo(request.getPageNo())
                .dataType(request.getDataType())
                .fcltyCd(request.getFcltyCd())
                .numOfRows(1)
                .build();
        return webClient.get()
                .uri(uriBuilder -> uriBuilder
                        .path("/data/getUniversity.do")
                        .queryParam("serviceKey", newRequest.getServiceKey())
                        .queryParam("pageNo", newRequest.getPageNo())
                        .queryParam("numOfRows", newRequest.getNumOfRows())
                        .queryParam("dataType", newRequest.getDataType())
                        .queryParam("Fclty_Cd", newRequest.getFcltyCd())
                        .build())
                .retrieve()
                .bodyToMono(UniversityApiResponse.class)
                .map(response -> {
